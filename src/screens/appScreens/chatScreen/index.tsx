...
const ChatScreen = (props: any) => {
  ...
  //     },
  //   );
  //   methodManageNotification("ON");
  //   return () => {
  //     appStateManage.remove();
  //     methodManageNotification("OFF");
  //   };
  // }, []);
  ...
  useEffect(() => {
    const handleAppStateChange = (nextAppState: any) => {
      ...
      if (nextAppState.match(/inactive|background/)) {
        ...
        methodManageNotification("OFF"); // This will be hit
      }
  ...
  const methodManageNotification = (status: string) => {
    let dic = {
      status: status,
    ...
    socketEmit(socketEvent.user_chat_screen, dic, (res) => {
      console.log("methodManageNotification-------", res);
    });
  };
  ...
  const methodReceiveMessage = (newMessage: ChatMessage): void => {
    ...
    if (
      hasDuplicate === -1 &&
      newMessage?.user_id === other_user_id &&
      ...
      // set conversation_id if not set in first time
      if (
        newMessage &&
        newMessage?.conversation_id &&
        !userData.conversation_id
      ) {
        userData.conversation_id = newMessage.conversation_id;
      }
    }
  ...
  //     }
  //     // set conversation_id if not set in first time
  //     if (res && res?.data?.length && !userData.conversation_id) {
  //       userData.conversation_id = res.data[0].conversation_id;
  //     }
  ...
  const methodGetAllChat = (listType: "before" | "after" = "after"): void => {
    const dic = {
      user_id: global?.userData?.id,
      other_user_id: other_user_id,
      type: "SINGLE",
    ...
    socketEmit(socketEvent.getchats, dic, (res: any) => {
      ...
      if (res && Array.isArray(res.data)) {
        ...
        if (listType === "after") {
          updatedMessages = [...res.data, ...messageData];
        } else {
          updatedMessages = [...messageData, ...res.data];
        }
        ...
        setChatHistoryData(uniqueMessages);

        // Update first/last message IDs
        firstMsgId = uniqueMessages.length ? uniqueMessages[0].id : 0;
        lastMsgId = uniqueMessages.length
          ? uniqueMessages[uniqueMessages.length - 1].id
          : 0;

        // Keep messageData in sync
        messageData = uniqueMessages;
      }
    });
  };
  ...
  const methodBlockUser = (): void => {
    ...
    socketEmit(socketEvent.user_block_unblock, dic, (data: any) => {
      console.log("user_block_unblock", data);
      if (data?.status) {
  ...
  const methodDeleteChat = (): void => {
    ...
    socketEmit(socketEvent.delete_chat, dic, (data: any) => {
      if (data?.status) {
        messageData = [];
        setChatHistoryData([]);
      }
  ...
  const handleReportUser = async (details: string) => {
    try {
      const payload = {
        other_user_id: other_user_id,
        message: details,
